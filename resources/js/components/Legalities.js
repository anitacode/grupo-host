...
const Legalities = ( ) => {
    ...
    const loadContents = async () =>{
        await fetching(config, global, 'legalities', setContents).then(oo =>{
            var currentID
                            ...
                            <div className=''>
                                <ListGroup className='cardspace p-0'>
                                    ...
                                    {contents.contents.map( (it) => {
                                        ...
                                        return <ListGroup.Item key={it.id} action active={it.id === id ? true : false} href={ link }>{ translate(it.title,0) }</ListGroup.Item>
                                    }
                                    )}
                                </ListGroup>
                            </div>
